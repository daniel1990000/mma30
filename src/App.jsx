import React, { useState, useEffect, useRef } from 'react';
import { Canvas, useFrame } from '@react-three/fiber';
import { OrbitControls } from '@react-three/drei';
import Arena from './components/Arena.jsx';
import Fighter from './components/Fighter.jsx';

function Scene({ keys }) {
  const playerRef = useRef();
  const aiRef = useRef();

  // State to control the current animation name. This is clean and efficient.
  const [playerAnimation, setPlayerAnimation] = useState('idle');

  // This effect triggers the animations based on key presses.
  useEffect(() => {
    // We only trigger a new animation if the fighter is idle.
    if (playerAnimation === 'idle') {
      if (keys['j']) {
        setPlayerAnimation('punch');
      } else if (keys['l']) {
        setPlayerAnimation('takedown');
      }
    }
  }, [keys, playerAnimation]);

  useFrame((state, delta) => {
    if (!playerRef.current || !aiRef.current) return;

    // -- MOVEMENT LOGIC (Unchanged) --
    const speed = 2;
    const velocity = { x: 0, z: 0 };
    if (keys['w']) velocity.z -= speed * delta;
    if (keys['s']) velocity.z += speed * delta;
    if (keys['a']) velocity.x -= speed * delta;
    if (keys['d']) velocity.x += speed * delta;
    playerRef.current.position.x += velocity.x;
    playerRef.current.position.z += velocity.z;

    // -- COLLISION DETECTION (We'll keep this here for later) --
    if (playerAnimation === 'punch') {
      const distance = playerRef.current.position.distanceTo(aiRef.current.position);
      if (distance < 1.5) {
        console.log("HIT!");
      }
    }
  });

  return (
    <>
      <OrbitControls />
      <ambientLight intensity={0.5} />
      <directionalLight position={[5, 10, 5]} intensity={1} castShadow />
      <Arena />

      <Fighter 
        ref={playerRef} 
        position={[0, 0.2, 2]} 
        rotation={[0, Math.PI, 0]} 
        color="red"
        animation={playerAnimation}
        // This function lets the fighter tell us when it's done animating.
        onAnimationComplete={() => setPlayerAnimation('idle')}
      />
      
      <Fighter 
        ref={aiRef} 
        position={[0, 0.2, -2]} 
        color="blue" 
        animation="idle" // The AI is always idle for now
      />
    </>
  );
}

function App() {
  const [keys, setKeys] = useState({});
  useEffect(() => {
    const handleKeyDown = (e) => setKeys((prev) => ({ ...prev, [e.key.toLowerCase()]: true }));
    const handleKeyUp = (e) => setKeys((prev) => ({ ...prev, [e.key.toLowerCase()]: false }));
    window.addEventListener('keydown', handleKeyDown);
    window.addEventListener('keyup', handleKeyUp);
    return () => {
      window.removeEventListener('keydown', handleKeyDown);
      window.removeEventListener('keyup', handleKeyUp);
    };
  }, []);

  return (
    <Canvas camera={{ position: [0, 5, 10] }} style={{ width: '100vw', height: '100vh' }} shadows>
      <Scene keys={keys} />
    </Canvas>
  );
}

export default App;